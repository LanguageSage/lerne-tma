    ...
    api.get('/study/stats')
      ...
      .catch((err) => {
        if (isMounted) {
          ...
          setError(tr("Не удалось загрузить аналитику"));
        }
      })
      .finally(() => {
        if (isMounted) {
  ...
  if (loading) {
    return (
      <div className="srs-loading-state">
        ...
        <p>{tr("Анализируем прогресс интервалов...")}</p>
      </div>
    );
  }
    ...
    <>
                ...
                <div className="srs-metrics-grid">
                  <div className="srs-metric-card primary">
                    ...
                    <div className="srs-metric-sub">
                      {retention == null ? tr("Нет данных") : retention >= 90 ? tr("🌟 Отличное удержание") : retention >= 80 ? tr("✅ Оптимальный темп") : tr("⚠️ Рекомендуется чаще повторять")}
                    </div>
                  </div>

                  <div className="srs-metric-card">
                    <div className="srs-metric-top">
                      <span className="srs-metric-label">{tr("Всего карточек")}</span>
                      <Award size={16} color="#60a5fa" />
                    </div>
                    <div className="srs-metric-val">{total}</div>
                    <div className="srs-metric-sub">{tr("В вашей базе")}</div>
                  </div>

                  <div className="srs-metric-card">
                    <div className="srs-metric-top">
                      <span className="srs-metric-label">{tr("Повторений (30д)")}</span>
                      <Clock size={16} color="#f59e0b" />
                    </div>
                    <div className="srs-metric-val">{stats?.total_reviews_30d || 0}</div>
                    <div className="srs-metric-sub">{tr("Оценок дано")}</div>
                  </div>
                </div>
                ...
                <div className="srs-section-card">
                  <div className="srs-section-header">
                    <h4>{tr("Зрелость памяти")}</h4>
                    <span className="srs-mature-badge">
                      {total > 0 ? tr("{{p0}}% изучено", { p0: Math.round(((youngCount + matureCount) / total) * 100) }) : '0%'}
                    </span>
                  </div>

                  {/* Stacked Progress Bar */}
                  <div className="srs-stacked-bar">
                    <div className="srs-bar-segment mature" style={{ width: `${maturePct}%` }} title={tr("Освоено: {{p0}}", { p0: matureCount })} />
                    <div className="srs-bar-segment young" style={{ width: `${youngPct}%` }} title={tr("Закрепляются: {{p0}}", { p0: youngCount })} />
                    <div className="srs-bar-segment learning" style={{ width: `${learnPct}%` }} title={tr("В процессе: {{p0}}", { p0: learningCount })} />
                    <div className="srs-bar-segment new" style={{ width: `${newPct}%` }} title={tr("Новые: {{p0}}", { p0: newCount })} />
                  </div>
                  ...
                  <div className="srs-legend-grid">
                    <div className="srs-legend-item">
                      ...
                      <div className="srs-legend-info">
                        <span className="srs-legend-name">{tr("Освоены (≥ 21 дн)")}</span>
                    ...
                    <div className="srs-legend-item">
                      ...
                      <div className="srs-legend-info">
                        <span className="srs-legend-name">{tr("Закрепляются (< 21 дн)")}</span>
                    ...
                    <div className="srs-legend-item">
                      ...
                      <div className="srs-legend-info">
                        <span className="srs-legend-name">{tr("В процессе (Учеба)")}</span>
                    ...
                    <div className="srs-legend-item">
                      ...
                      <div className="srs-legend-info">
                        <span className="srs-legend-name">{tr("Новые (Не начаты)")}</span>
                  ...
                  {leechCount > 0 && (
                    <div className="srs-leech-notice">
                      ...
                      <span><strong>{leechCount}</strong> {leechCount === 1 ? tr("сложная карточка (Leech)") : tr("сложных карточек (Leech)")}{' '}{tr("— рекомендуем пересмотреть формулировки.")}</span>
                    </div>
                  )}
                </div>
                ...
                {forecast.length > 0 && (
                  <div className="srs-section-card">
                    <div className="srs-section-header">
                      <h4>
                        <Calendar size={16} style={{ marginRight: '6px', verticalAlign: 'middle' }} />{tr("План повторений на 7 дней")}{' '}</h4>
                    </div>
                        ...
                        return (
                          <div key={item.date || idx} className={`srs-forecast-col ${isToday ? 'today' : ''}`}>
                            ...
                            <span className="srs-forecast-day">
                              {isToday ? tr("Сегодня") : item.day_name}
                            </span>
                          </div>
                        );
                      })}
        ...
        >
          <div className="srs-modal-header">
            <div className="srs-title-wrapper">
              ...
              <div>
                <h3>{tr("Статистика памяти (SRS)")}</h3>
                <p>{tr("Эффективность интервального повторения")}</p>
              </div>
            </div>
