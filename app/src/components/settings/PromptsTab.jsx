...
export const PromptsTab = () => {
  ...
  const handleCreateNew = () => {
    ...
    if (activeCategoryTab === 'exam') {
      ...
      activeTranslation = tr("Генерируй карточки для изучения грамматики языка {{p0}}. Оборачивай проверяемую грамматическую форму в скобки [слово] или {слово} в предложении на лицевой стороне (например: Ich sehe [den] Hund). На обратной стороне напиши подробный и развернутый грамматический разбор правила.", { p0: langInfo.name });
      activeContext = activeTranslation;
      ...
      <p className="field-hint" style={{ marginBottom: '15px' }}>
        {activeCategoryTab === 'exam'
          ...
          : (activeCategoryTab === 'trainer' 
              ? tr("Инструкции ИИ для авто-создания карточек-тренажеров со скобками [слово] или {слово} и подробным разбором правил.", {  })
